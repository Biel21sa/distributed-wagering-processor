...
export class WagerTransaction {
  ...
  /**
   * Internal-only factory for the OPENING transaction created when a wallet
   * is opened with an initial balance. This bypasses the public `create`
   * guard (which forbids OPENING) because OPENING is generated by the system,
   * never by an API/message caller. The resulting transaction is already
   * PROCESSED and represents the CREDIT of the initial balance.
   */
  static openWallet(props: {
    id: string;
    walletId: string;
    playerId: string;
    money: Money;
    ...
    if (
      ...
      throw new Error(
        'Opening amount must be positive',
      );
    }
